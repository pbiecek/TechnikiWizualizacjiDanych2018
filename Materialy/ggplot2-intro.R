...
continents <- group_by(countries, continent) %>% 
  na.omit %>% 
  summarise(death.rate = sum(death.rate),
...
ggplot(continents, aes(x = birth.rate, y = death.rate, size = population, label = continent, color = n.countries)) +
  geom_point() +
...
ggplot(data = countries, aes(x = continent, y = death.rate)) +
  ...
  geom_point(data = group_by(countries, continent) %>% summarise(mean.death.rate = mean(death.rate)),
             aes(x = continent, y = mean.death.rate), color = "red", size = 4)
...
na.omit(countries) %>% 
  mutate(label = ifelse(birth.rate %in% range(birth.rate) | death.rate %in% range(death.rate),
                        country, "")) %>% 
  ggplot(aes(x = birth.rate, y = death.rate, label = label)) +
  geom_point() +
  geom_text_repel()
...
# I. Na jednym wykresie przedstaw rozklad cen i wieku mieszkan dla a) calego Wroclawia b) poszczegolnych dzielnic.
