...
read.table("./nsp-makeover/gr2/Lukasz_Brzozowski/Lukasz_Brzozowski/ksieza.csv", sep = "\t", header = TRUE) %>% 
  mutate(Miejsce = c("Świat", "Świat", "Europa", "Europa", "Polska", "Polska")) %>% 
  melt %>% 
  mutate(variable = as.numeric(sub("X", "", variable)),
         Miejsce = factor(Miejsce),
         Miejsce = factor(Miejsce, levels = levels(Miejsce)[c(3, 1, 2)])) %>% 
  filter(Rodzaj == "księża") %>% 
  ggplot(aes(x = variable, y = value, fill = Miejsce)) + 
